...
void GameScene::Draw()
{
    ...
    SpriteBasic::GetInstance()->SetCommonRenderSetting();
    pGameHUD_->Draw();
    pSpriteCursorSwitchGuide_->Draw();
...
{
    auto tm = Tako::TextureManager::GetInstance();
    tm->LoadTexture(Global::ResourcePath::Texture::kKeyGuideSwitchCursor);
    tm->LoadTexture(Global::ResourcePath::Texture::kKeyGuideSwitchCursorPad);

    pSpriteCursorSwitchGuide_ = std::make_unique<Tako::Sprite>();
    pSpriteCursorSwitchGuide_->Initialize(Global::ResourcePath::Texture::kKeyGuideSwitchCursor);
    pSpriteCursorSwitchGuide_->SetPos({ 5_vw, 5_vw });
    Tako::Vector2 size = pSpriteCursorSwitchGuide_->GetSize();
    pSpriteCursorSwitchGuide_->SetSize(size * 0.5f);

    InputAwareSprite::Entry entry;
    entry.pSprite_ = pSpriteCursorSwitchGuide_.get();
    entry.handleKeyboard_ = Tako::TextureManager::GetInstance()->GetSRVIndex(Global::ResourcePath::Texture::kKeyGuideSwitchCursor);
    entry.handleGamepad_ = Tako::TextureManager::GetInstance()->GetSRVIndex(Global::ResourcePath::Texture::kKeyGuideSwitchCursorPad);
    entry.sizeKeyboard_ = size * 0.5f;
    entry.sizeGamepad_ = size * 0.5f;
